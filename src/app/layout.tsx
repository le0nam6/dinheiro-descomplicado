...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // A home cresce junto com a tela em vez de parar em 1024px: num monitor
  // grande sobravam quase 500px mortos de cada lado. O post fica de fora
  // porque lá a coluna estreita é proposital — 67 caracteres por linha.
  ...
  return (
    <html lang="pt-BR" className={`${inter.variable} ${manrope.variable}`}>
      <head>
        ...
        {/* Google AdSense: o script precisa estar no <head> de toda pagina, tanto
            pra verificacao de propriedade quanto pra veicular anuncio depois. */}
        ...
        {/* Microsoft Clarity: mapa de calor e gravacao de sessao. O GA4 conta
            quantos chegaram; o Clarity mostra o que fizeram na pagina. */}
        <Script id="clarity-init" strategy="afterInteractive">{`
          (function(c,l,a,r,i,t,y){
            c[a]=c[a]||function(){(c[a].q=c[a].q||[]).push(arguments)};
            t=l.createElement(r);t.async=1;t.src="https://www.clarity.ms/tag/"+i;
            y=l.getElementsByTagName(r)[0];y.parentNode.insertBefore(t,y);
          })(window, document, "clarity", "script", "yjwjq6xaix");
        `}</Script>
      </head>
      <body>
        {isLP ? children : (
          <>
            ...
            <QuotesTicker />
            <Header />
            <main className={`${isHome ? 'max-w-5xl lg:max-w-6xl xl:max-w-[1360px] 2xl:max-w-[1520px]' : isWide ? 'max-w-5xl' : 'max-w-4xl'} mx-auto px-4 sm:px-6 xl:px-10 py-12`}>{children}</main>
            <Footer />
          </>
        )}
      </body>
